...
void puzzle_life(GameState *state, int id) {
    ...
    state->puzzles_solved[id] = 1;   // markera som löst
}

void puzzle_damage(GameState *state, int id) {
    print("You've solved the puzzle!\n");
    print("+5 DMG \n\n");
    change_damage(&state->player, +5);
    state->puzzles_solved[id] = 1;   // markera som löst
}
...
void solve_puzzle(GameState *state, const Room* room) {
    ...
    switch (room->id) {
        case 1: puzzle1(state); print_meny(state->player.current_room); break;
        case 2: puzzle2(state); print_meny(state->player.current_room); break;
        case 3: puzzle3(state); print_meny(state->player.current_room); break;
        case 4: puzzle4(state); print_meny(state->player.current_room); break;
        case 5: print("There is no puzzle, only beautiful scenery\n\n"); print_meny(state->player.current_room); break;
        case 6: puzzle5(state); print_meny(state->player.current_room); break;
        case 7: puzzle6(state); print_meny(state->player.current_room); break;
        case 8: puzzle7(state); print_meny(state->player.current_room); break;
        case 9: puzzle8(state); print_meny(state->player.current_room); break;
        default:
